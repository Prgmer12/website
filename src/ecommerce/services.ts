import { supabase } from "../Admin/lib/supabase";import type {
  Category,
  Product,
  Review,
  Address,
...
export async function fetchCategories(): Promise<Category[]> {
  const { data, error } = await supabase
    ...
    .select("*")
    .eq("is_active", true)
    .order("sort_order", { ascending: true });  if (error) throw error;
  return data ?? [];
}
...
export async function addReview(
  ...
  const { data, error } = await supabase
    .from("reviews")    .insert({ product_id: productId, rating, title, body, verified: true })
    .select()
    .single();
  if (error) throw error;
  return data;
...
export async function fetchWishlistProductIds(): Promise<number[]> {
  const { data, error } = await supabase.from("wishlist").select("product_id");  if (error) throw error;
...
export async function fetchWishlistProducts(): Promise<Product[]> {
  const { data: wishRows, error: wErr } = await supabase
    .from("wishlist")
    .select("product_id");  if (wErr) throw wErr;
  const ids = (wishRows ?? []).map((w) => w.product_id);
  if (ids.length === 0) return [];
  const { data: products, error: pErr } = await supabase
    .from("products")
    .select("*")
    .in("id", ids);  if (pErr) throw pErr;
  return products ?? [];
}
...
export async function createOrder(
  ...
  const { error: iErr } = await supabase.from("order_items").insert(rows);  if (iErr) throw iErr;
...
export function calculateShippingEstimate(
  pincode: string,
  subtotal: number,
): ShippingEstimate {  const FREE_SHIPPING_THRESHOLD = 2000;
  ...
  if (!isServiceable) {
    return {
      ...
      courier: "Standard",      available: false,
    };
  }
